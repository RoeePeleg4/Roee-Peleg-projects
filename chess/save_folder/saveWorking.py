...
def creation():
    ...
    black_queen = Character("queen" , "black" , (3,0)) #(None,None)
    ...
    while running:
        ...
        if(is_check):
            ...
            if king == None:
                ...
                return
            for i in range(len(arr_char)):
                list = updateMove(piece , king)
                if list!=None:
                    arr_char[i].setArrMoveOptions(list)
                    print(arr_char[i].getArrMoveOptions())
            isCheckMate = True
            for k in range(len(arr_char)):
                if king.getColor() == arr_char[k].getColor():
                    if arr_char[k].getX() != None and arr_char[k].getY() != None and len(arr_char[k].getArrMoveOptions()) > 0:
                        isCheckMate = False
                        break
            if isCheckMate:
