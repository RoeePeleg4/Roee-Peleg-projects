...
def updateOptionsEat(arr_char , player_color):
    castelingLocations = []
    for i in range(len(arr_char)):
        """if arr_char[i].getType() == "king" and isCheck(arr_char , arr_char[i].getColor()) == False:
            castelingLocations = getMovingInCastelling(arr_char[i] , arr_char[i] , player_color)"""
...
def creation(side):
    ...
    opp_side_bishop2 = Character ("bishop", opp_side , (None, None) , 0) #(5,0)
    ...
    opp_side_night2 = Character("knight" , opp_side , (None , None) , 0) #(6 , 0)
...
def canMove(piece , arr_char , position , king_by_piece_color , player_color):
    ...
    for i in range(len(arr_char)):
        if arr_char[i].getX() == arr_char[index].getX() and arr_char[i].getY() == arr_char[index].getY() and arr_char[index].getId() != arr_char[i].getId() and arr_char[index].getColor()!=arr_char[i].getColor():
            ...
            index_of_none = i
            if 31 == index_of_none:
                print(piece.getType() , piece.getId() , piece.getColor() , "peleg")
            break
...
def minimax(array_to_change , depth , maximizing_player , player_color = "white"):
    ...
    if maximizing_player:
        ...
        for piece in array_to_change:
            ...
            is_in_check = isCheck(arr_char , getOppositeColor(player_color))
            for move in list_of_moving_options:
                ...
                if is_in_check == False and piece.getType() == "king":
                    castelling_locations = getMovingInCastelling(getKing(piece.getColor() , array_to_change) , piece , piece.getColor())
...
def getBestMove(arr_char1 , player_color):
    ...
    for i in range(len(arr_char1)):
        ...
        for move in valid_options:
            ...
            save_right_rook_location = (None, None)
            save_left_rook_location = (None, None)
            if not is_in_check and copy_list[i].getType() == "king":
                castelling_locations = getMovingInCastelling(getKing(copy_list[i].getColor() , copy_list) , copy_list[i] , copy_list[i].getColor())
                for i in range(len(castelling_locations)):
                    if castelling_locations[i] == move:
                        if (copy_list[i].getX() + 2) == move[0]: 
                            rightRook = getRightRook(copy_list[i].getColor() , copy_list)
                            rightRook.setX((copy_list[i].getX() + 2) - 1)
                            save_right_rook_location = (rightRook.getX() , rightRook.getY())
                            has_castelled_main = True
                        elif (copy_list[i].getX() - 2) == move[0]:
                            leftRook = getLeftRook(copy_list[i].getColor() , copy_list)
                            leftRook.setX((copy_list[i].getX() - 2) + 1)
                            save_left_rook_location = (leftRook.getX() , leftRook.getY())
                            has_castelled_main = True
                        else:
                            print("[SYSTEM] error with bot castelling")
