            ...
            if piece != None and TURN == piece.getColor():
                if piece.getIsClicked() == True:
                    ...
                    if currentPiece.getType() == "pawn":
                        print("pawn")
                    delete_by_index = []
                    #checking for casteling and adding their locations to an array of all the casteling locations
                    castelingLocations = []
                    castelling = isCastelling(currentPiece.getColor() , arr_char)
                    print(castelling)
                    if castelling == "big small":
                        castelingLocations.append((king_turn.getX() + 2 , king_turn.getY()))
                        castelingLocations.append((king_turn.getX() - 2 , king_turn.getY()))
                        OPTIONS.append((king_turn.getX() + 2 , king_turn.getY()))
                        OPTIONS.append((king_turn.getX() - 2 , king_turn.getY()))
                    elif castelling == "big":
                        castelingLocations.append((king_turn.getX() - 2 , king_turn.getY()))
                        OPTIONS.append((king_turn.getX() - 2 , king_turn.getY()))
