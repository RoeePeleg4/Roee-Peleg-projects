...
def updateOptionsEat(arr_char , player_color):
    castelingLocations = []
    for i in range(len(arr_char)):
        if arr_char[i].getType() == "king":
            castelingLocations = getMovingInCastelling(getKing(arr_char[i].getColor() , arr_char) , arr_char[i] , player_color)
    ...
    if maximizing_player:
        ...
        for piece in array_to_change:
            ...
            for move in list_of_moving_options:
                ...
                if has_castelled:
                    if rightRook is not None and save_right_rook_location is not (None, None):
                        rightRook.setX(save_right_rook_location[0])
                        rightRook.setY(save_right_rook_location[1])
                    if leftRook is not None and save_left_rook_location is not (None, None):
...
def getBestMove(arr_char1 , player_color):
    ...
    if piece_to_move.getType() is "pawn" and best_move[1]==7:
