...
class Button():
    ...
    def __init__(self, middle_cordinate_for_placement = (0,0) , text = "text" , color = (255,255,255) , shape = "rectangle", pixel_size = 100 , text_font = "Arial" , radius = None , width_spacing = 1 , height_spacing = 1 , text_spacing = 0.1):
        ...
        self.height = self.height_spacing  * text_height * len(self.text_list)
    ...
    def inSideRect(self , cordinate):
        top_left_x , top_left_y = self.middleCords[0] - (self.width / 2) , self.middleCords[1] - (self.width / 2)
        bottom_right_x , bottom_right_y = self.middleCords[0] + (self.width / 2) , self.middleCords[1] + (self.width / 2)
    ...
    def showButton(self):
        if self.shape == "rectangle":
            pygame.draw.rect(window , self.color , (self.middleCords[0] - (self.width / 2) , self.middleCords[1] - (self.height / 2) , self.width , self.height))
        ...
        for i in range(len(self.text_list)):
            ...
            if len(self.text_list) % 2 == 0:
                ...
                if (i + 1) <= len(self.text_list) / 2:
                    y = self.middleCords[1] - self.getHeightByLines((i + 1))#(i + 1) * text_height
                #if bellow half
                else:
                    y = self.middleCords[1] + self.getHeightByLines((i - (len(self.text_list) / 2)) , len(self.text_list) / 2)#(i - (len(self.text_list) / 2)) * text_height
            #if it is odd
            else:
                #if middle
                if (i + 0.5) == len(self.text_list) / 2:
                    y = self.middleCords[1] - (0.5 * text_height)
                #if above middle
                elif i < len(self.text_list) / 2:
                    y = self.middleCords[1] - self.getHeightByLines((len(self.text_list) / 2) - i)#((len(self.text_list) / 2) - i) * text_height 
                #if bellow middle
                else:
                    y = self.middleCords[1] + self.getHeightByLines(i - (len(self.text_list) / 2) , (len(self.text_list) / 2) - 0.5)#(i - (len(self.text_list) / 2)) * text_height
            draw_txt(self.text_list[i] , self.text_font , (30,144,255) , self.middleCords[0] - text_width / 2 , y)
...
def creation(side):
    ...
    for i in range(8):
        ...
        side_pawn = Character("pawn", side , (i , 6))
        opp_side_pawn = Character("pawn", opp_side , (i , 1))
        arr_char.append(opp_side_pawn)
        arr_char.append(side_pawn)
    side_bishop1 = Character ("bishop", side , (2,7)) 
    side_bishop2 = Character ("bishop", side, (5, 7))
    opp_side_bishop1 = Character ("bishop", opp_side , (2, 0))
    opp_side_bishop2 = Character ("bishop", opp_side , (5, 0))
    arr_char.append(side_bishop1)
    arr_char.append(side_bishop2)
    arr_char.append(opp_side_bishop1)
    arr_char.append(opp_side_bishop2)
    side_rook1 = Character("rook" , side , (0 , 7))
    side_rook2 = Character("rook" , side , (7 , 7))
    opp_side_rook1 = Character("rook" , opp_side , (0 , 0))
    opp_side_rook2 = Character("rook" , opp_side , (7 , 0))
    arr_char.append(side_rook1)
    arr_char.append(side_rook2)
    arr_char.append(opp_side_rook1)
    arr_char.append(opp_side_rook2)
    side_night1 = Character("knight" , side , (1 , 7))
    side_night2 = Character("knight" , side , (6 , 7))
    opp_side_night1 = Character("knight" , opp_side , (1 , 0))
    opp_side_night2 = Character("knight" , opp_side , (6 , 0))
    arr_char.append(side_night1)
    arr_char.append(side_night2)
    arr_char.append(opp_side_night1)
    arr_char.append(opp_side_night2)
    side_queen = Character("queen" , side , (3 , 7))
    opp_side_queen = Character("queen" , opp_side , (3 , 0))
    arr_char.append(side_queen)
    arr_char.append(opp_side_queen)
    side_king = Character("king" , side , (4 , 7))
    opp_side_king = Character("king" , opp_side , (4 , 0))
...
def getValidPositions(list , current_piece , king , player_color):
...
def main():
    ...
    #creation of buttons
    """"to split by \n ,  string_str.split("\n") returns an array"""
    sameComputerButton = Button((700,500) , "1v1\non\nthe\nsame\ncomputer" , (255,255,255) , "rectangle" , 30)
    buttons_list = []
    ...
    while running:
        ...
        if isInChoosingScreen:
            if sameComputerButton.getFirstClick():
        ...
        if (check_pressed_mouse(MOUSE_PRESSED) == "pressed"):
            ...
            if piece != None and TURN == piece.getColor():
                ...
                if piece.getIsClicked() == True:
                    ...
                    OPTIONS = getValidPositions(OPTIONS , currentPiece , king_turn , player_color)
                    OPTIONS_EAT = getValidPositions(OPTIONS_EAT , currentPiece , king_turn , player_color)
