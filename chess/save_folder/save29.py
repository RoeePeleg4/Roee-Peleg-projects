...
def updateOptionsEat(arr_char , player_color):
    castelingLocations = []
    for i in range(len(arr_char)):
        arr_char[i].setArrMoveOptions(getPlaces(arr_char[i] , arr_char , player_color)[1] + getPlaces(arr_char[i] , arr_char , player_color)[0] + castelingLocations)
...
def creation(side):
    ...
    opp_side_bishop2 = Character ("bishop", opp_side , (5,0) , 0) #(5,0)
    ...
    opp_side_night2 = Character("knight" , opp_side , (6 , 0) , 0) #(6 , 0)
...
def canMove(piece , arr_char , position , king_by_piece_color , player_color):
    ...
    index_of_none = -1
    for i in range(len(arr_char)):
        if arr_char[i].getX() == arr_char[index].getX() and arr_char[i].getY() == arr_char[index].getY() and arr_char[index].getId() != arr_char[i].getId() and arr_char[index].getColor()!=arr_char[i].getColor():
...
def isCastelling(color , arr_char , player_color):
    ...
    king = getKing(color ,arr_char)
    if king.getIsMoved() == True:
        return "cant"
    elif canBig and canSmall and LeftRook is not None and RightRook is not None and RightRook.getIsMoved() == False and LeftRook.getIsMoved() == False and ((king.getX() + 1 , king.getY()) and (king.getX() - 1 , king.getY())) in king.getArrMoveOptions():
        return "big small"
    elif canSmall and RightRook is not None and RightRook.getIsMoved() == False and (king.getX() + 1 , king.getY()) in king.getArrMoveOptions():
        return "small"
    elif canBig and LeftRook is not None and LeftRook.getIsMoved() == False and (king.getX() - 1 , king.getY()) in king.getArrMoveOptions():
        return "big"
    return "cant"

#a function that is getting all of the castelling moving options and returning array of them
def getMovingInCastelling(king , player_color):
    castelling = isCastelling(king.getColor() , arr_char , player_color)
    castelingLocations = []
...
def evaluate(arr_of_players):
    ...
    total_points_for_no_eating_white = 0#8 * dict_points["pawn"] + 2 * dict_points["rook"] + 2 * dict_points["knight"] + 2 * dict_points["bishop"] + dict_points["king"] + dict_points["queen"]
    total_points_for_no_eating_black = 0#8 * dict_points["pawn"] + 2 * dict_points["rook"] + 2 * dict_points["knight"] + 2 * dict_points["bishop"] + dict_points["king"] + dict_points["queen"]
    for i in range(len(arr_of_players)):
        if (arr_of_players[i].getX() or arr_of_players[i].getY()) != None:
    ...
    print(total_points_for_no_eating_white - total_points_for_no_eating_black)
...
def minimax(array_to_change , depth , alpha , beta , maximizing_player , player_color = "white"):
    if depth == 0 or game_over():
        return evaluate(array_to_change)
    #means if white
    if maximizing_player:
        max_eval = float('-inf')
        is_in_check_before_move = isCheck(array_to_change , getOppositeColor(player_color))
        for piece in array_to_change:
            ...
            if piece.getType() == "king" and not is_in_check_before_move:
                castelling_locations = getMovingInCastelling(getKing(piece.getColor() , array_to_change) , player_color) #might need to change something about player color
                list_of_moving_options += castelling_locations
            for move in list_of_moving_options:
                save_location = piece.getX() , piece.getY()
                #save for the rooks locations
                has_castelled = False
                rightRook = None
                leftRook = None
                save_right_rook_location = (None, None)
                save_left_rook_location = (None, None)
                if is_in_check_before_move == False and piece.getType() == "king":
                    rightRook = getRightRook(piece.getColor() , array_to_change)
                    leftRook = getLeftRook(piece.getColor() , array_to_change)
                    for i in range(len(castelling_locations)):
                        if castelling_locations[i] == move:
                            if (piece.getX() + 2) == move[0] and not rightRook.getIsMoved(): 
                                rightRook.setX((piece.getX() + 2) - 1)
                                save_right_rook_location = (rightRook.getX() , rightRook.getY())
                                has_castelled = True
                            elif (piece.getX() - 2) == move[0] and not leftRook.getIsMoved():
                                leftRook.setX((piece.getX() - 2) + 1)
                                save_left_rook_location = (leftRook.getX() , leftRook.getY())
                                has_castelled = True
                            else:
                                print("[SYSTEM] error with bot castelling")
                if not isLocationOpen(move , array_to_change):
                    piece_to_remove = getPieceByLocation(move , array_to_change)
                    if piece_to_remove.getColor() == "black":
                        piece_to_remove.setX(None)
                        piece_to_remove.setY(None)
                piece.setX(move[0])
                piece.setY(move[1])
                piece.setIsClicked()
                updateOptionsEat(array_to_change , player_color)
                eval = minimax(array_to_change, depth - 1 , alpha , beta , False)
                max_eval = max(max_eval, eval)
                alpha = max(alpha , eval)
                piece.setIsClicked()
                piece.setX(save_location[0])
                piece.setY(save_location[1])
                #reset rook location after finished castelling
                if has_castelled:
                    if rightRook != None and save_right_rook_location != (None, None):
                        rightRook.setX(save_right_rook_location[0])
                        rightRook.setY(save_right_rook_location[1])
                    if leftRook != None and save_left_rook_location != (None, None):
                        leftRook.setX(save_left_rook_location[0])
                        leftRook.setY(save_left_rook_location[1])
                    has_castelled = False
                updateOptionsEat(array_to_change , player_color)
                #pruning
                if beta <= alpha:
    ...
    #means if black
        ...
        for piece in array_to_change:
            ...
            list_of_moving_options = getValidPositions(piece.getArrMoveOptions() , piece , getKing(piece.getColor() ,array_to_change) , piece.getColor() , array_to_change)
            if piece.getType() == "king" and not is_in_check_before_move:
                castelling_locations = getMovingInCastelling(getKing(piece.getColor() , array_to_change) , player_color)
                list_of_moving_options +=castelling_locations
            for move in list_of_moving_options:
                save_location = piece.getX() , piece.getY()
                #save for the rooks locations
                ...
                if not isLocationOpen(move , array_to_change):
                    piece_to_remove = getPieceByLocation(move , array_to_change)
                    if piece_to_remove.getColor() == "white":
                        piece_to_remove.setX(None)
                        piece_to_remove.setY(None)
                piece.setX(move[0])
                piece.setY(move[1])
                piece.setIsClicked()
                updateOptionsEat(array_to_change , player_color)
                eval = minimax(array_to_change  , depth - 1, alpha , beta, True)
...
#a function that returns the rook of the type of castelling based on the move (before moving the piece)
def getRookByMove(king , move , arr_char):
    king_pos = (king.getX() , king.getY())
    if (move[0] + 2) == king_pos[0]:
        return getRightRook(king.getColor() , arr_char)
    elif (move[0] - 2) == king_pos[0]:
        return getLeftRook(king.getColor() , arr_char)
    else:
        #means the move was not castelling
        return None

#a function that checks if a move by the bot cause castelleing
#when it is a bot a castelling will alwats
def getBestMove(arr_char1 , player_color):
    # Example usage
    best_move = None
    best_eval_white = float('-inf')
    best_eval_black = float('inf')
    piece_to_move = None
    copy_list = copy.deepcopy(arr_char1)
    save_copy = None
    is_in_check_before_move = isCheck(arr_char1 , getOppositeColor(player_color))
    for i in range(len(arr_char1)):
        ...
        if arr_char1[i].getType() == "king" and not is_in_check_before_move:
            castelling_locations = getMovingInCastelling(getKing(copy_list[i].getColor() , copy_list) , player_color)
            valid_options+=castelling_locations
        for move in valid_options:
            # Make the move
            # Update the board
            #copy of the pieces list 
            #varibles for castelling
            #save for the rooks locations
            has_castelled_main = False
            rightRook = None
            leftRook = None
            save_right_rook_location = (None, None)
            save_left_rook_location = (None, None)
            if not is_in_check_before_move and copy_list[i].getType() == "king":
                for h in range(len(castelling_locations)):
                    if castelling_locations[h] == move:
                        if (copy_list[h].getX() + 2) == move[0]: 
                            rightRook = getRightRook(copy_list[h].getColor() , copy_list)
                            rightRook.setX((copy_list[h].getX() + 2) - 1)
                            save_right_rook_location = (rightRook.getX() , rightRook.getY())
                            has_castelled_main = True
                        elif (copy_list[h].getX() - 2) == move[0]:
                            leftRook = getLeftRook(copy_list[h].getColor() , copy_list)
                            leftRook.setX((copy_list[h].getX() - 2) + 1)
                            save_left_rook_location = (leftRook.getX() , leftRook.getY())
                            has_castelled_main = True
                        else:
                            print("[SYSTEM] error with bot castelling")
            for g in range(len(copy_list)):
                if arr_char1[i].getId() == copy_list[g].getId():
                    copy_list[g].setX(move[0])
                    copy_list[g].setY(move[1])
                    save_copy = copy_list[g]
                    break
            if save_copy is None:
                print("[SYSTEM] an error has occured")
                return 1
            updateOptionsEat(copy_list , player_color)
            print(move , i)
            if player_color == "white":
                eval = minimax(copy_list , 2 , float('-inf') , float('inf') , True)
            else:
                eval = minimax(copy_list , 2 , float('-inf') , float('inf') , False)
            save_copy.setX(arr_char1[i].getX())
            save_copy.setY(arr_char1[i].getY())
            #reset rook location after finished castelling
            if has_castelled_main:
                if rightRook != None and save_right_rook_location != (None, None):
                    rightRook.setX(save_right_rook_location[0])
                    rightRook.setY(save_right_rook_location[1])
                if leftRook != None and save_left_rook_location != (None, None):
                ...
                has_castelled_main = False
            save_copy = None
            if arr_char1[i].getColor() == "white":
                if eval > best_eval_white and len(valid_options)!=0:
                    piece_to_move = arr_char1[i]
                    best_eval_white = eval
                    best_move = move
            elif arr_char1[i].getColor() == "black":
                if eval < best_eval_black and len(valid_options)!=0:
                    piece_to_move = arr_char1[i]
                    best_eval_black = eval
                    best_move = move
    if best_eval_white==float('-inf') and best_eval_black==float('inf'):
        return None , None
    #when a bot the other side is always the down side means y=7
    if piece_to_move.getType() == "pawn" and best_move[1]==7:
        piece_to_move.setType("queen")
...
def main():
    ...
    while running:
        ...
        if isAgainstBot and TURN is not player_color:
            ...
            if (move and bot_piece) is None:
                print("[SYSTEM] no place to go to")
                return 
        ...
        if (check_pressed_mouse(MOUSE_PRESSED) == "pressed"):
            ...
            if piece != None and TURN == piece.getColor():
                ...
                if piece.getIsClicked() == True:
                    ...
                    castelingLocations = []
                    if currentPiece.getType() == "king" and not is_check:
                        castelingLocations = getMovingInCastelling(king_turn , player_color)
                        for i in range(len(castelingLocations)):
                            if castelingLocations[i] not in OPTIONS:
