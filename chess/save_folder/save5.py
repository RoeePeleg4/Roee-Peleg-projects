...
def deffendObjects(king, arr_char):
    ...
    dangering_objects = []
    for i in range(len(arr_char)):
        if (arr_char[i].getColor() != king.getColor() and (king.getX() , king.getY()) in arr_char[i].getArrMoveOptions()):
            dangering_objects.append(arr_char[i])
    for i in range(len(dangering_objects)):
        for g in range(len(arr_char)):
            if (arr_char[g].getColor() == king.getColor()):
...
def main():
    ...
    while running:
        ...
        is_check = (opp_color != None and isCheck(arr_char , TURN))
        if (is_check):
            pass
            #print("check")
        ...
        if(is_check):
            ...
            dict_moving_options = deffendObjects(king , arr_char)
            for key in dict_moving_options.keys():
                print(key.getColor())
            ...
            isCheckMate = True"""
            for i in range(len(arr_char)):
                if king.getColor() == arr_char[i].getColor():
                    if arr_char[i].getX() != None and arr_char[i].getY() != None and len(arr_char[i].getArrMoveOptions()) > 0:
                        print(arr_char[i].getType() , arr_char[i].getX() , arr_char[i].getY())
                        isCheckMate = False
                        break
            if isCheckMate:
            ...
            if piece != None and TURN == piece.getColor():
                if piece.getIsClicked() == True:
                    ...
                    for i in range(len(OPTIONS)):
                        if not canMove(arr_char[i] , arr_char , OPTIONS[i] , king_turn):
                    ...
                    for g in range(len(OPTIONS_EAT)):
                        if not canMove(arr_char[i] , arr_char , OPTIONS_EAT[g] , king_turn):
