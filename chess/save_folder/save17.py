...
def creation(side):
    ...
    side_queen = Character("queen" , side , (3 , 7) , 0)
...
def getBestMove(arr_char , player_color):
    # Example usage
    best_move = None
    best_eval = float('-inf')
    piece_to_move = None
    copy_list = copy_arr(arr_char)
    save_copy = None
    isMoveMakingChess = False
    for piece in arr_char:
        print(piece.getId())
        if piece.getColor() == player_color:
            continue
        for move in piece.getArrMoveOptions():
            # Make the move
            # Update the board
            #copy of the pieces list 
            for i in range(len(arr_char)):
                if piece.getId() == copy_list[i].getId():
                    copy_list[i].setX(move[0])
                    copy_list[i].setY(move[1])
                    if isCheck(arr_char , getOppositeColor(player_color)):
                        copy_list[i].setX(piece.getX())
                        copy_list[i].setY(piece.getY())
                        isMoveMakingChess = True
                    save_copy = copy_list[i]
                    break
            #skip the iteration if the move is making chess
            if isMoveMakingChess:
                continue
            if save_copy is None:
                print("[SYSTEM] an error has occured")
                return 1
            if player_color == "white":
                eval = minimax(copy_list, 3, True)
            else:
                eval = minimax(copy_list, 3, False)
            save_copy.setX(piece.getX())
            save_copy.setY(piece.getY())
            isMoveMakingChess = False
            save_copy = None
            if eval > best_eval:
                piece_to_move = piece
